...
function NEAT(config) {
  ...
  this.mutate = function () {
    // Parses every creature's genes passes them to the mutation function and sets their new (mutated) genes.
    for (let i = 0; i < this.populationSize; i++) {
  ...
  this.crossover = function () {
    // Takes two creature's genes flattens them and passes them to the crossover function.
    for (let i = 0; i < this.populationSize; i++) {
      this.oldCreatures = Object.assign([], this.creatures);
      let parentx = this.pickCreature();
      let parenty = this.pickCreature();

      let genes = this.crossoverMethod(
        parentx.flattenGenes(),
        parenty.flattenGenes(),
      );
      this.creatures[i].setFlattenedGenes(genes);
    }
  };
  ...
  this.pickCreature = function () {
    // Normalizes every creature's score (fitness) and and returns a creature based on their fitness value.
    let sum = 0;
    ...
    for (let i = 0; i < this.oldCreatures.length; i++) {
      this.oldCreatures[i].fitness =
        Math.pow(this.oldCreatures[i].score, 2) / sum;
    }
    let index = 0;
    let r = Math.random();
    while (r > 0) {
  ...
  this.setFitness = function (fitness, index) {
    // Sets a creature's score. This will then be normalized for actual fitness value.
    this.creatures[index].score = fitness;
  };

  this.feedForward = function () {
    // Feeds forward every creature's network.
  ...
  this.doGen = function () {
    // Does 1 fast generation with crossover and mutation.
    this.crossover();
    this.mutate();
    this.generation++;
  ...
  this.bestCreature = function () {
    // Returns the index of the best creature from the previous generation.
    let index = 0;
    let max = -Infinity;
    for (let i = 0; i < this.oldCreatures.length; i++) {
  ...
  this.getDesicions = function () {
    // Returns every creature's desicion index in an array.
    let result = [];
  ...
  this.setInputs = function (array, index) {
    // Sets the inputs of the creature indexed as "index".
  ...
  this.getTensorflowModel = function (index) {
    // Generate the requested tensorflow model.
    if (index) {
...
function Creature(model) {
  ...
  this.flattenGenes = function () {
    // Flattens the genes of the creature's genes and returns them as an array.
    let genes = [];
    ...
    for (let i = 0; i < this.network.layers.length - 1; i++) {
      for (let w = 0; w < this.network.layers[i].nodes.length; w++) {
        for (
          let e = 0;
          e < this.network.layers[i].nodes[w].weights.length;
          e++
        ) {
  ...
  this.setFlattenedGenes = function (genes) {
    // Sets an array of weights as the creature's genes.
    for (let i = 0; i < this.network.layers.length - 1; i++) {
      for (let w = 0; w < this.network.layers[i].nodes.length; w++) {
        for (
          let e = 0;
  ...
  this.feedForward = function () {
    // Feeds forward the creature's network.
    this.network.feedForward();
  };

  this.setInputs = function (inputs) {
    // Sets the inputs of the creature.
    this.network.layers[0].setValues(inputs);
  };

  this.desicion = function () {
    // Some spaghetti code that returns the desicion of the creature.
    let index = -1;
    let max = -Infinity;
    for (
      let i = 0;
      i < this.network.layers[this.network.layers.length - 1].nodes.length;
      i++
    ) {
      if (
        this.network.layers[this.network.layers.length - 1].nodes[i].value > max
      ) {
        max =
          this.network.layers[this.network.layers.length - 1].nodes[i].value;
        index = i;
      }
    }
    return index;
...
function Network(model) {
  ...
  for (let i = 0; i < model.length; i++) {
    // Init all the layers.
    this.layers.push(
      new Layer(model[i].nodeCount, model[i].type, model[i].activationfunc),
    );
  }
  ...
  this.getTensorflowModel = function () {
    // Generates a tensorflow model from the current network.
    ...
    for (let i = 1; i < this.model.length - 1; i++) {
      previous = tf.layers
        .dense({ units: this.model[i].nodeCount, activation: "relu" })
        .apply(previous);
    }
    // Initialize the output layer.
    let output = tf.layers
      .dense({
        units: this.model[this.model.length - 1].nodeCount,
        activation: "softmax",
      })
      .apply(previous);
    ...
    for (let i = 1; i < this.model.length; i++) {
      model.layers[i].setWeights([
        tf.tensor(
          weights[i - 1],
          [weights[i - 1].length, weights[i - 1][0].length],
          (dtype = "float32"),
        ),
        tf.fill([this.model[i].nodeCount], 0, (dtype = "float32")),
      ]);
    }
...
function Layer(nodeCount, type, activationfunc) {
  // A layer component of a network with nodes and bias node.
  this.nodes = [];
  this.bias = undefined;
  this.type = type;
  ...
  this.feedForward = function (layer) {
    // Feeds forward the layers values to the specified layer.
  ...
  this.getValues = function () {
    // Returns the values of the nodes in the layer as an array.
    let result = [];
    for (let i = 0; i < this.nodes.length; i++) {
  ...
  this.setValues = function (values) {
    // Sets an array as the nodes values.
    for (let i = 0; i < this.nodes.length; i++) {
...
let crossover = {
  ...
  SLICE: function (genesx, genesy) {
    ...
    let end =
      Math.floor(Math.random() * (genesx.length - start + 2)) + start + 1;
    let cutPart = genesx.splice(start, end);

    Array.prototype.splice.apply(
      genesy,
      [start, cutPart.length].concat(cutPart),
    );
...
let mutate = {
  // Mutation function (More to come!).
